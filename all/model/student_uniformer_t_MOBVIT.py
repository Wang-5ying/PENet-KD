...
from backbone.mobilevit import MobileViT_XXS
...
class SRAA(nn.Module):
    def load_pret(self, pre_model1):
        ...
        self.resnet_depth.load_state_dict(new_state_dict3, strict=True)
    ...
    def __init__(self):
        ...
        # 64 128 320 512   - > 24 48 64 320
        self.resnet = MobileViT_XXS()
        self.resnet_depth = MobileViT_XXS()
        self.raa1 = RRAA(in_channel=24, out_channel=24, h=80, w=80)
        self.raa2 = RRAA(in_channel=48, out_channel=48, h=40, w=40)
        self.raa3 = RRAA(in_channel=64, out_channel=64, h=20, w=20)
        self.raa4 = RRAA(in_channel=320, out_channel=320, h=10, w=10)
        ...
        self.glo = nn.Conv2d(320, 16, 1)
        self.end1 = End(in_channel=16)

        self.sup1 = nn.Conv2d(16, 1, 3, 1, 1)
        self.sup2 = nn.Conv2d(16, 1, 3, 1, 1)
        self.sup3 = nn.Conv2d(16, 1, 3, 1, 1)

        self.b1 = BasicConv2d(24, 16, 3, 1, 1)
        ...
        self.edge1 = edge(24, 1)
        self.edge2 = edge(64, 1)

        self.before1 = BasicConv2d(320, 64, 3, 1, 1)
        self.r1 = BasicConv2d(64, 48, 3, 1, 1)
        ...
        self.fenliang1 = BasicConv2d(320, 16, 1)
        self.fenliang2 = BasicConv2d(320 + 64, 16, 1)
        self.fenliang3 = BasicConv2d(320 + 64 + 48, 16, 1)
        self.fenliang4 = BasicConv2d(320 + 64 + 48 + 24, 16, 1)
        ...
        self.er1 = nn.Conv2d(24, 64, 1)
        self.er2 = nn.Conv2d(48, 128, 1)
        self.er3 = nn.Conv2d(64, 256, 1)
        self.er4 = nn.Conv2d(320, 512, 1)

        self.ed1 = nn.Conv2d(24, 64, 1)
        self.ed2 = nn.Conv2d(48, 128, 1)
        self.ed3 = nn.Conv2d(64, 256, 1)
        self.ed4 = nn.Conv2d(320, 512, 1)

        self.pool1 = nn.AdaptiveAvgPool2d(20)
        self.pool2 = nn.AdaptiveAvgPool2d(40)
        self.pool3 = nn.AdaptiveAvgPool2d(80)
    def forward(self, r, d):
        ...
        glo2 = self.cca2(glo2)
        res1 = self.b2(res)
        res1 = F.interpolate(res1, 40, mode='bilinear', align_corners=True)
        a1 = F.interpolate(rd4, 40, mode='bilinear', align_corners=True)
        a2 = F.interpolate(rd3, 40, mode='bilinear', align_corners=True)
        a12 = self.fenliang2(torch.cat((a1, a2), dim=1))
        a12cw = self.cw1(self.beforecw1(a12), self.beforecw1(a12))
        a12cw2 = self.aftercw1(F.interpolate(a12cw, size=40))
        ...
        res2 = self.b3(res1)
        res2 = F.interpolate(res2, 80, mode='bilinear', align_corners=True)
        a1 = F.interpolate(rd4, 80, mode='bilinear', align_corners=True)
        a2 = F.interpolate(rd3, 80, mode='bilinear', align_corners=True)
        a3 = F.interpolate(rd2, 80, mode='bilinear', align_corners=True)
        a123 = self.fenliang3(torch.cat((a1, a2, a3), dim=1))
        a123cw3 = self.cw1(self.beforecw1(a123), self.beforecw1(a123))
        a123cw3 = self.aftercw1(F.interpolate(a123cw3, size=80))
        ...
        res3 = self.b3(res2)
        res3 = F.interpolate(res3, 160, mode='bilinear', align_corners=True)
        a1 = F.interpolate(rd4, 160, mode='bilinear', align_corners=True)
        a2 = F.interpolate(rd3, 160, mode='bilinear', align_corners=True)
        a3 = F.interpolate(rd2, 160, mode='bilinear', align_corners=True)
        a4 = F.interpolate(rd1, 160, mode='bilinear', align_corners=True)
        a1234 = self.fenliang4(torch.cat((a1, a2, a3, a4), dim=1))
        a1234cw4 = self.cw1(self.beforecw1(a1234), self.beforecw1(a1234))
        a1234cw4 = self.aftercw1(F.interpolate(a1234cw4, size=160))
        ...
        kdr = []
        kdr.append(self.er1(rlayer_features[0]))
        kdr.append(self.er2(rlayer_features[1]))
        kdr.append(self.er3(rlayer_features[2]))
        kdr.append(self.er4(rlayer_features[3]))

        kdd = []
        kdd.append(self.ed1(tlayer_features[0]))
        kdd.append(self.ed2(tlayer_features[1]))
        kdd.append(self.ed3(tlayer_features[2]))
        kdd.append(self.ed4(tlayer_features[3]))

        adj = []
        adji = self.pool1(decode[1])
        adj.append(adji)
        adji = self.pool2(decode[2])
        adj.append(adji)
        adji = self.pool3(decode[3])
        adj.append(adji)
        return res, b2, b3, b4, edge1, edge2, kdr, kdd, decode, adj
