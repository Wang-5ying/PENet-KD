...
from mmseg.models.backbones.mix_transformer import mit_b3
...
class SRAA(nn.Module):
    ...
    def __init__(self):
        ...
        # 64 128 320 512
        self.resnet = mit_b3()
        self.resnet_depth = mit_b3()
        self.raa1 = RRAA(in_channel=64, out_channel=64, h=96, w=96)
        self.raa2 = RRAA(in_channel=128, out_channel=128, h=48, w=48)
        self.raa3 = RRAA(in_channel=320, out_channel=320, h=24, w=24)
        self.raa4 = RRAA(in_channel=512, out_channel=512, h=12, w=12)
        ...
        self.fenliang2 = BasicConv2d(512 + 320, 16, 1)
        self.fenliang3 = BasicConv2d(512 + 320 + 128, 16, 1)
        self.fenliang4 = BasicConv2d(512 + 320 + 128 + 64, 16, 1)
        ...
        self.cca1 = Channel_aware_CoordAtt(16, 16, 10, 10)
        self.cca2 = Channel_aware_CoordAtt(16, 16, 20, 20)
        self.cca3 = Channel_aware_CoordAtt(16, 16, 40, 40)
        self.cca4 = Channel_aware_CoordAtt(16, 16, 80, 80)
    ...
    def forward(self, r, d):
        ...
        b1 = self.b1(rd1)
        b1 = F.interpolate(b1, 10, mode='bilinear', align_corners=True)
        a1 = F.interpolate(rd4, 10, mode='bilinear', align_corners=True)
        a1 = self.fenliang1(a1)
        a1cw = self.cw1(self.beforecw1(a1), self.beforecw1(a1))
        a1cw1 = self.aftercw1(F.interpolate(a1cw, size=10))
        ...
        glo2 = self.cca2(glo2)
        res1 = self.b2(res)
        res1 = F.interpolate(res1, 20, mode='bilinear', align_corners=True)
        a1 = F.interpolate(rd4, 20, mode='bilinear', align_corners=True)
        a2 = F.interpolate(rd3, 20, mode='bilinear', align_corners=True)
        a12 = self.fenliang2(torch.cat((a1, a2), dim=1))
        a12cw = self.cw1(self.beforecw1(a12), self.beforecw1(a12))
        a12cw2 = self.aftercw1(F.interpolate(a12cw, size=20))
        ...
        res3 = self.b3(res2)
        res3 = F.interpolate(res3, 80, mode='bilinear', align_corners=True)
        ...
        a4 = F.interpolate(rd1, 80, mode='bilinear', align_corners=True)
        a1234 = self.fenliang4(torch.cat((a1, a2, a3, a4), dim=1))
        a1234cw4 = self.cw1(self.beforecw1(a1234), self.beforecw1(a1234))
        a1234cw4 = self.aftercw1(F.interpolate(a1234cw4, size=80))
...
if __name__ == '__main__':
    a = torch.randn(2, 3, 320, 320).cuda()
    b = torch.randn(2, 3, 320, 320).cuda()
    model = SRAA()
    model.cuda()
    # model.load_pret("/media/wby/shuju/seg_pre/segformer.b1.512x512.ade.160k.pth")
    out = model(a, b)
    # end = time.time()
